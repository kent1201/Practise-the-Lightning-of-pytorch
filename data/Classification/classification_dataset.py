...
from data.Classification import compute_mean_std, DataTransform
...
class ClassificationDataset(pl.LightningDataModule):
    ...
    def setup(self, stage='fit'):
        if stage == "fit":
            self.train_transform = DataTransform(args=self.args, mean=self.mean, std=self.std, mode="train")
            self.val_transform = DataTransform(args=self.args, mean=self.mean, std=self.std, mode="val")
            self.train_dataset = K2Dataset(self.args, "train", self.train_transform)
